...
/// [`MerklePatriciaTrie`] is a Merkle Tree that is well suited for both basic and advanced usage.
///
...
#[derive(Clone, Debug)]
pub struct MerklePatriciaTrie<T: Hasher> {
    current_working_tree: PartialTree<T>,
    history: Vec<PartialTree<T>>,
    uncommitted_leaves: Vec<T::Hash>,
}

impl<T: Hasher> Default for MerklePatriciaTrie<T> {
    fn default() -> Self {
        Self::new()
    }
}

impl<T: Hasher> MerklePatriciaTrie<T> {
    /// Creates a new instance of Merkle Tree. Requires a hash algorithm to be specified.
    ///
    /// # Examples
    ///
    /// ```
    /// use rs_merkle::{MerklePatriciaTrie, algorithms::Sha256};
    ///
    /// let merkle_tree: MerklePatriciaTrie<Sha256> = MerkleTree::new();
    ///
    /// let another_merkle_tree = MerklePatriciaTrie::<Sha256>::new();
    /// ```
    pub fn new() -> Self {
        Self {
    ...
    ///
    /// ```
    /// # use rs_merkle::{MerklePatriciaTrie, MerkleProof, algorithms::Sha256, Hasher, Error, utils};
    ...
    /// ];
    ///
    /// let merkle_tree = MerklePatriciaTrie::<Sha256>::from_leaves(&leaves);
    /// # Ok(())
    /// # }
    ...
    /// Similar to [`MerklePatriciaTrie::root`], but returns a hex encoded string instead of
    /// [`Hasher::Hash`].
    ///
    /// ## Examples
    ///
    ...
    /// to be applied to the root, [`MerklePatriciaTrie::commit`] method should be called first. To get the
    /// root of the new tree without applying the changes, you can use
    /// [`MerklePatriciaTrie::uncommitted_root`]
    ///
    /// # Examples
    ///
    ...
    /// # use std::convert::TryFrom;
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// let mut merkle_tree = MerklePatriciaTrie::<Sha256>::new();
    ...
    /// ```
    ///
    /// Inserts also can be chained with [`MerklePatriciaTrie::commit`] for convenience:
    ///
    /// ```
    /// # use rs_merkle::{MerklePatriciaTrie, MerkleProof, algorithms::Sha256, Hasher, Error, utils};
    /// # use std::convert::TryFrom;
    ...
    /// Appends leaves to the tree. Behaves similarly to [`MerklePatriciaTrie::insert`], but for a list of
    ...
    /// Commits the changes made by [`MerklePatriciaTrie::insert`] and [`MerkleTree::append`]
    /// and modifies the root.
    /// Commits are saved to the history, so the tree can be rolled back to any previous commit
    /// using [`MerklePatriciaTrie::rollback`]
    ///
    /// ## Examples
    ///
    /// ```
    ...
    /// Will return the same hash as [`MerklePatriciaTrie::root`] after [`MerkleTree::commit`]
    ///
    /// For examples, please check [`MerklePatriciaTrie::uncommitted_root_hex`]
    ...
    /// Will return the same hash as [`MerklePatriciaTrie::root_hex`] after [`MerkleTree::commit`]
    ///
    /// ### Examples
    ///
    /// ```
    ...
    /// Clears all uncommitted changes made by [`MerklePatriciaTrie::insert`] and [`MerkleTree::append`]
